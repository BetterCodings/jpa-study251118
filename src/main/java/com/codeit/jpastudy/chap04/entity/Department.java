...
@Getter
// JPA 양방향 연관관계 매핑에서 연관관계 필드는 toString에서 제외해야합니다. (순환 참조 발생)
...
public class Department {
    ...
    // 엔티티 안에서만 사용하는 가상의 컬럼입니다.
    // 상대방 엔티티의 갱신에 관여할 수 없기 때문에 단순히 읽기 전용(조회)으로만 사용하는 것을 권장
//    @OneToMany(mappedBy = "department", cascade = CascadeType.REMOVE) // 연관 관계 엔티티의 필드명을 작성
    @OneToMany(mappedBy = "department", orphanRemoval = true, cascade = CascadeType.ALL) // 연관 관계 엔티티의 필드명을 작성
    ...
    /*
        ...
        부모를 수정해서 반영하면 그 안에 있는 자식도 함께 반영된다.

        # CascadeType.REMOVE
        엔티티를 제거할 때, 연관된 하위 엔티티도 모두 제거한다.

        //////////////////////////////////////////////////////////////////////
        아래 두 개는 (Entity Manager)가 필요하다.
